...
\begin{rem}
    ...
    \begin{itemize}
        ...
        \item
            $\priority(n, v)$ is \textit{global} in a sense that the values of $\priority(n, v)$ calculated at node $w$ and $w'$ must be identical for it only depends on $n$, the hash function $G_i$ and the constant $P$ and it does not depend on $w$. 
    \end{itemize}
\end{rem}

...
\begin{defn}[Prepare]
    Let $b$ be a ballot.
    A node is said to vote, accept, or confirm that $b$ is \textit{prepared} if and only if it votes, accepts, or confirms \textit{abort} $b'$ for all $b' \lnsim b$, respectively.
\end{defn}

\begin{rem}
    This approach is different from the white paper's approach.
    ...
    The definition of voting states that one votes for a statement \textit{if and only if} the four conditions have been met.  
    However, this new rule will change that.
\end{rem}

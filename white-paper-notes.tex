...
\begin{proof}
    \todo[inline,caption={}]{
        Make this proof more understandable.
    }
    ...
    \begin{itemize}
        \item
            ...
            Since $\ev{V, Q}$ enjoys quorum intersection, $B$ is a DSet by Theorem~\ref{befouled_dset}, so $\ev{V, Q}^B$ enjoys quorum intersection.
        \item
            Claim 2: $\ev{V, Q}$ does not enjoy quorum intersection despite $B$.
            $U \setminus B$ is a quorum in $\ev{V, Q}^B$ as mentioned above and we assumed that $(V \setminus B) \setminus S^{+}$ is a quorum in $\ev{V, Q}^B$.
    ...
    Then $S^{-} = \emptyset$, and we are done.
\end{proof}
...
\begin{proof}
    ...
    Thus we may experience a gradual increase in the number of nodes that accepted $a$.

...
One way to interpret this lemma is:

\begin{itemize}
    \item
        We choose to care about a quorum containing at least one intact node.
        This is reasonable because, for instance, we might not care much about what quorums consisting only of ill-behaved nodes confirm.
    \item
        We hope that a statement such a quorum accepts and confirms will be accepted and confirmed by \textit{all} intact nodes.
        This is reasonable because we hope that \textit{all} intact nodes in the network agree and confirm the same thing all the time.
    \item
        One way to make a node $v$ outside such a quorum agree and confirm a statement is for $U$ to be $v$-blocking.
    \item
        \todo[inline,caption={}]{
        }
\end{itemize}

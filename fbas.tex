...
\begin{cor}\label{set_of_intact_nodes_quorum}
    In an FBAS with quorum intersection, the set of intact nodes is either empty or a quorum.
\end{cor}

\begin{proof}
    The set $B$ of befouled nodes is a DSet by Theorem~\ref{befouled_dset}.
    By the definition of a DSet, $B^c$ is either empty or a quorum.
    Therefore, the set of intact nodes, if not empty, is a quorum.
\end{proof}

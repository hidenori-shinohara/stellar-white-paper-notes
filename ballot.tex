...
\begin{defn}[Transition Type 2]
    ...
    \begin{enumerate}
        ...
        \item % 3
            If $\varphi = \text{PREPARE}, c = 0, b \leq h$, and neither $p \gnsim h$ nor $p' \gnsim h$, then set $c$ to the lowest ballot satisfiying $b \leq c \lnsim h$.
        \item % 4
        ...
        \item % 6
            If $\varphi = \text{CONFIRM}$ and $v$ accepts more commit messages or raise $b$, then let $h'$ be the highest ballot such that $v$ accepts all $\{ \textit{commit } b' \mid b \lnsim b' \lnsim h' \}$  (if any).
        ...
        \item % 8
            If $\varphi = \{ \text{PREPARE}, \text{CONFIRM} \}$ and $b < h$, then set $b \leftarrow h$.
        \item % 9

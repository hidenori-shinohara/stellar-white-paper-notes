...
\begin{thm}\label{nonzero_h_if_confirmed}
...
\begin{proof}
    ...
    \begin{enumerate}[label=(\roman*)]
        \item
            $v$ has confirmed a ballot as prepared, and $h \ne \textbf{0}$.
            But a new messages lets $v$ set $h \leftarrow \textbf{0}$.
        \item
            $v$ has not confirmed a ballot as prepared, and $h = \textbf{0}$.
            But a new messages lets $v$ confirm a ballot as prepared without updating value of $h$.
        \item
            $v$ has not confirmed a ballot as prepared, and $h \ne \textbf{0}$.
            But a new messages lets $v$ confirm a ballot as prepared and set $h \leftarrow \textbf{0}$.
    \end{enumerate}
    We will use the fact that none of the actions sets $h \leftarrow \textbf{0}$.
    Thus (i) and (ii) are impossible.
    Moreover, if $h = \textbf{0}$ and a new message lets $v$ confirm a ballot as prepared, Step (2) forces $v$ to update the value of $h$ a nonzero value.

    Thus it is impossible to reach a state where $v$ has confirmed a ballot as prepared, and $h = \textbf{0}$.
\end{proof}
...
\begin{proof}
    ...
    By taking the contrapositive of Theorem~\ref{nonzero_h_if_confirmed}, we know that if $h = \textbf{0}$, then $v$ has not confirmed any ballot as prepared.
    ...
    This means we must take Step (2) in Transition Type 2(\ref{transition_type_2}), and that means we will udpate $h$ to the highest ballot confirmed as prepared.
